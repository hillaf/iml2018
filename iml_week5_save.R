...
cluster <- function(input, k, feature_means){
  ...
  c <- c(rep(1:k, length.out=n))
  input <- cbind(input, c)
  data <- data.frame(input)
  new <- TRUE
  
  # assign random feature means unless given as a parameter
  if (is.null(feature_means)){
    feature_means <- aggregate(.~c, data=data[1:k,], mean)
  }
  ...
  while (new == TRUE){
    ...
    # calculate euclidian distance between each observation and the cluster centroids
    ...
    feature_means <- aggregate(.~c, data=data, mean)
  }
...
par(mfrow=c(2,1))
plot(inp[,1:2], col=clu[1:n,3], main="3a")
...
means <- matrix(c(train$x[4,], train$x[6,], train$x[8,], train$x[3,], train$x[1,], train$x[14,], train$x[16,], train$x[18,], train$x[5,], train$x[2,]), nrow=10, byrow=TRUE)
c <- c(1:10)
means <- cbind(c, means)
