...
calculate_log_loss <- function(pre, td, smc, smoothed_x_0, smoothed_x_1, smoothed_x_2){
  ll <- vector()
  x <- td[,1:2]
  results <- td[,3]
...
logres_logloss <- function(pre, td, probabilities){
  ll <- vector()
  results <- td[,3]
...
for(i in a){
  ...
  # 2 a log losses for naive bayes and logistic regression
  # naive Bayes
  # obtain Laplace smoothed conditional probability distributions for each class
  ...
  logloss <- calculate_log_loss(predictions, test_data, smoothed_c, smx0, smx1, smx2)
  ...
  logres_log_losses <- c(logres_log_losses, logres_logloss(pr, test_data, prob))
  ...
  interaction_log_losses <- c(interaction_log_losses, logres_logloss(pr, test_data, prob))
  interaction_error_rates <- c(interaction_error_rates, calculate_error(pr, test_data))
}


plot(1:9, naive_bayes_log_losses, type="l", ylim=c(1300, 20000), ylab="Log loss", axes=FALSE)
